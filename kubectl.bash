...
function var_usage() {
  ...
  CLOUDSDK_API_KEY=<api key> (base64 encoded)
...
echo `echo ${key} | base64 -d > /tmp/key.json`
echo "Running: gcloud auth activate-service-account --key-file=/tmp/key.json"
gcloud auth activate-service-account --key-file=/tmp/key.json || exit
